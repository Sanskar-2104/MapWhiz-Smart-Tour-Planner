...
import { DaySelector } from "@/components/DaySelector";
import { DayDetails } from "@/components/DayDetails";
import styles from "./ItineraryGenerator.module.css"; // CSS module we added
      ...
      },
      {
        ...
        places: [
          ...
          },
          {
            ...
            name: "Nahargarh Fort",
            description:
              "Perched on the edge of the Aravalli Hills, this fort offers breathtaking panoramic views of Jaipur. The perfect spot to watch the sunset.",
            latitude: 26.9389,
            longitude: 75.8164,
            startTime: "16:00",
            endTime: "18:30",
...
export const ItineraryGenerator = () => {
  ...
  // ensure selectedDay is valid on mount
  useEffect(() => {
    if (tripData.trip.days && tripData.trip.days.length > 0) {
      setSelectedDay((prev) =>
        tripData.trip.days.some((d) => d.dayIndex === prev) ? prev : tripData.trip.days[0].dayIndex
      );
    }
  }, []);
    ...
    const handleKey = (e: KeyboardEvent) => {
      if (e.key === "ArrowRight") {
        const nextIndex = Math.min(
          selectedDay + 1,
          tripData.trip.days[tripData.trip.days.length - 1].dayIndex
        );
        setSelectedDay(nextIndex);
      } else if (e.key === "ArrowLeft") {
        const prevIndex = Math.max(selectedDay - 1, tripData.trip.days[0].dayIndex);
                  ...
                  {tripData.trip.days.map((d) => {
                    ...
                    return (
                      <div
                        ...
                        onKeyDown={(e) => { if (e.key === "Enter") setSelectedDay(d.dayIndex); }}
                      ...
                      >
                        ...
                        <div className={styles.dayExcerpt}>
                          {d.notes}
                        </div>
                      </div>
                    );
                    ...
                    <div className={styles.dayHeaderRow}>
                      ...
                      <div>
                        ...
                        <div className={styles.dayDate}>
                          {new Date(currentDay.date).toLocaleDateString(undefined, { weekday: "long", month: "long", day: "numeric", year: "numeric" })}
                        </div>
                      </div>
                    </div>
                    ...
                    <div key={p.id} className={styles.placeCard}>
                      <div className={styles.placeTopRow}>
                        ...
                        <div style={{flex:1}}>
                          ...
                          <div className={styles.placeTime}>⏱ {p.startTime} - {p.endTime}</div>
                        </div>
                      </div>
                      ...
                      <div className={styles.placeFooter}>
                        <div className={styles.coords}>📍 {p.latitude.toFixed(4)}, {p.longitude.toFixed(4)}</div>
                        <div className={styles.actions}>
                          <button className={styles.btnPrimary}>➜ Directions</button>
                          <button className={styles.btnSecondary}>ℹ️ Details</button>
                        </div>
                      </div>
                    </div>
                  ))}
